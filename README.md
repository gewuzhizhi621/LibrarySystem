...
本项目是一个基于 **C++ + EasyX 图形库** 开发的图书管理系统，采用图形化界面完成图书馆常见业务管理功能。

系统支持 **管理员端** 和 **读者端** 两种身份登录。管理员可以进行图书管理、借阅管理、读者管理、数据统计、操作日志查看和数据备份；读者可以查询图书、查看个人借阅记录和修改密码。

项目数据采用 **CSV 文件** 进行本地存储，不依赖数据库，结构清晰，便于维护，适合作为 C++ 课程设计、大一程序设计项目或 EasyX 图形界面练习项目。
...
> 如果图片不显示，请检查图片路径和文件名是否与 README 中保持一致。
...
![管理员主界面](docs/images/admin_menu.png)

### 图书管理

![图书管理](docs/images/book_manage.png)

### 借阅管理

![借阅管理](docs/images/borrow_manage.png)

### 读者管理

![读者管理](docs/images/reader_manage.png)

### 数据统计中心

![数据统计中心](docs/images/statistics.png)
...
![读者服务中心](docs/images/reader_menu.png)
...
* 使用 CSV 作为轻量级本地数据库
* 支持管理员和读者双身份系统
* 支持图书、读者、借阅、统计、日志、备份等完整功能
* 代码采用模块化设计，结构清晰
* 数据文件独立存放，便于维护和修改
* 适合课程设计、项目答辩和基础系统开发练习
...
* 请勿删除 `Easyx/` 目录
* 程序运行时需要能够读取 CSV 文件
* 如果数据不更新，请检查程序运行目录是否为项目根目录
* 如果中文乱码，建议将源码和 CSV 文件保存为 UTF-8 编码
* 如果背景图片不显示，请检查 `picture/` 目录是否存在
...
如果使用 Visual Studio 运行，可以将调试工作目录设置为：
...
#### 6.1 Visual Studio 项目字符集设置为 Unicode
...
该选项可以让 Visual Studio 按 UTF-8 编码处理源代码中的中文字符。
...
EasyX 和 Windows API 中显示中文时，建议使用宽字符字符串。
